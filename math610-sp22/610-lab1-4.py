#solve -(ku')' + qu = f with u(0) = 0 and either u'(L) = 1 or u(L) = 1
#for u'(L) = 1, test with k(x) = 1 + x, q(x) = 1, u(x) = x*(x-1), and L = 1
#for u(L) = 1, test with k(x) = 1 + x, q(x) = 1, u(x) = x*(2-x), and L = 1
...
mesh_size = domain_end/number_of_elements
boundary_flags = [0, 1] #0 for dirichlet, 1 for neumann
boundary_values = [0, 1] #values that u or u' takes at the boundary points 
...
#left boundary
if boundary_flags[0] == 0: #if dirichlet
    global_matrix[0,0] = 1
    global_matrix[0,1] = 0
    load_vector[0] = 0 #EDIT THIS
else: #neumann
    load_vector[0] -= 0 #EDIT THIS
#right boundary
if boundary_flags[1] == 0: #if dirichlet
    global_matrix[-1,-1] = 1
    global_matrix[-1,-2] = 0
    load_vector[-1] = 0 #EDIT THIS
else: #neumann
    load_vector[-1] += 0 #EDIT THIS
