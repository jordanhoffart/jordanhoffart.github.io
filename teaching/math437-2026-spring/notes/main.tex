...
\begin{theorem}[Convergence of fixed-point methods]\label{thm:fixed_point}
  Suppose $g$ is differentiable, and there exists $k$ such that $|g'(x)| \leq k < 1$ for all $x \in [a,b]$.
  Then, $g$ has a unique fixed point, and the iterative method \eqref{eq:iterative_method} converges to this point for any initial value $x_0 \in [a,b]$.
\end{theorem}
\begin{proof}
  ...
  Therefore, $|x_m - x_n| \to 0$ as $m, n \to \infty$, so the sequence $(x_n)_n$ is a Cauchy sequence of real numbers.
  The sequence therefore must converge to some number $p$.
