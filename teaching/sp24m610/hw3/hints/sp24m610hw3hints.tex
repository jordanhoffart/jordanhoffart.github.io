...
\end{equation}
for $j = 0,\dots,n-1$.
This gives us $n + 1$ basis functions defiend so far.
...
\begin{align}
  \psi_{2n+1}(x) & = \begin{cases} \phi_1^{0}(x) & x \in (x_0,x_1) \\  0 & \text{otherwise} \end{cases}, \\
  \psi_{2n+2}(x) & = \begin{cases} \phi_3^{n-1}(x) & x \in (x_{n-1},x_n) \\  0 & \text{otherwise} \end{cases}.
\end{align}
This gives us a grand total of $m = 2n+2$ basis functions.
