...
W = lambda x : # notice that the variable here is x, not t
...
integrate = lambda integrand : sum([integrand(quad_pts[q])*quad_wts[q] for q in range(len(quad_pts))])
...
for k in range(1,len(elements)-1):
  ...
  for i in range(len(basis)):
    Fk[i] = integrate(integrand) # replace this
    for j in range(len(basis)):
      Ak[i,j] = integrate(integrand) # replace this
...
with open("data.txt") as f:
  for j in range(N):
    f.write(",".join([plot_nodes[j], Wh[j]])+"\n")
...
for k in range(1,len(elements)-1):
  ...
  W0 = Wh[k-1]
  W1 = Wh[k]
  integrand = lambda x : (W(refmap(x)) - W0*basis[0](x) - W1*basis[1](x))**2*dmap(x)
...
print(",".join([N,h,np.sqrt(L2_sq),np.sqrt(H1_sq),Linf]))
