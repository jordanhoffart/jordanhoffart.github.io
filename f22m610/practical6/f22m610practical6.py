...
for kh in [1,2,3,4]:
    ...
    for e in es:
        ...
        for ir in [0,1,2]:
            for ic in [0,1,2]:
                A[e[ir],e[ic]] += detB/2*np.dot(np.matmul(dphi[ir],invB),np.matmul(dphi[ic],invB))
                for iq in range(nq):
                    M[e[ir],e[ic]] += detB*wq[iq]*phi[ir](xq[iq])*phi[ic](xq[iq])
            for iq in range(nq):
    ...
    for it in range(nt):
        ...
        for e in es:
            ...
            for ir in [0,1,2]:
                for iq in range(nq):
                    F1[e[ir]] += wq[iq]*f((it+1)*dt,v0+np.matmul(B,xq[iq]))*phi[ir](xq[iq])*detB
